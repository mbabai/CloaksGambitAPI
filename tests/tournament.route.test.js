...
describe('tournaments routes', () => {
  ...
  test('details requires tournament membership', async () => {
    process.env.NODE_ENV = 'development';
    const app = createApp();

    resolveSessionFromRequest.mockResolvedValueOnce({
      userId: 'host-details',
      username: 'HostDetails',
      isGuest: true,
      authenticated: false,
    });
    const createRes = await request(app)
      .post('/api/v1/tournaments/create')
      .send({ label: 'Details Cup' });

    resolveSessionFromRequest.mockResolvedValueOnce({
      userId: 'spectator-outsider',
      username: 'Outsider',
      isGuest: true,
      authenticated: false,
    });
    ...
    expect(outsiderRes.body.message).toMatch(/join this tournament/i);
  });

  test('admin can list and delete tournaments', async () => {
    process.env.NODE_ENV = 'development';
    const app = createApp();
    ...
    ensureAdminRequest.mockResolvedValue({ userId: 'admin-1', email: 'marcellbabai@gmail.com' });
    ...
    expect(deleteRes.body.tournamentId).toBe(createRes.body.tournament.id);
  });
});
