...
- Live `Game` and `Match` records are stored in memory while active. Completed games and matches are persisted to MongoDB history collections. `User`, `ServerConfig`, `Simulation`, and `SimulationGame` stay Mongo-backed throughout.
