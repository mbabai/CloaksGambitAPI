...
class BotClient {
  ...
  start() {
    ...
    this.socket.on('connect', () => console.log('[bot] connected', { userId: this.userId }));
  ...
  handleUpdate(payload) {
    ...
    if (!controller) {
      ...
      controller = new GameController(this.serverUrl, payload.gameId, this.userId, this.token, this.socket);
...
async function startBotClient({ serverUrl, difficulty = 'easy', secret = '', heartbeatInterval = 10000 } = {}) {
  ...
  const client = new BotClient(serverUrl, registration.token, registration.userId);
  client.start();
