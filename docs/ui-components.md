...
- `.cg-elo-badge` sizing is driven by CSS custom properties (`--cg-elo-badge-width`, `--cg-elo-badge-font-size`, etc.) populated by `createEloBadge`. The standard badge stays compact relative to the player-name row: preserve the badge font size, shrink the shield around it, and place the badge after the player name when used inside ranked name rows.
