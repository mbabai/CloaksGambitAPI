...
This document lists the REST endpoints exposed by the Cloaks Gambit API. Real-time
match and move data is streamed to clients via WebSocket events.

## Users
- `POST /api/v1/users/getList` - Retrieve users with optional filters
- `POST /api/v1/users/getDetails` - Get details for a specific user. Missing
  historical users return a sanitized placeholder instead of a `404`.
...
Real-time features use a Socket.IO connection. Clients should connect with their
authenticated cookie-backed session to receive personalized events:
