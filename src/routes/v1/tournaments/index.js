...
router.post('/admin/list', async (req, res) => {
  try {
    ...
    const tournaments = await listTournamentsForAdmin({ status: req.body?.status || 'all' });
...
router.post('/admin/delete', async (req, res) => {
  try {
    const adminSession = await ensureAdminRequest(req, res);
    if (!adminSession) return;
    const result = await deleteTournamentCascade({ tournamentId: req.body?.tournamentId });
